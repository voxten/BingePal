...
export default function UserProfilePage() {
    ...
    const profileId = params.id; // Extracts the target ID from the URL string (/profile/tt123)
    ...
    // Check if the currently logged-in account matches the profile collection being viewed
    ...
    return (
        <div className={`min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 transition-colors duration-300 selection:bg-indigo-500 selection:text-white ${darkMode ? 'dark' : ''}`}>
                    ...
                    <div className="flex items-center gap-3.5">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider -mt-0.5">
                                {isOwner ? "My Collection" : "Shared Collection View"}
                            </p>
                        </div>
                    </div>
            ...
            <main className="container mx-auto px-4 py-8">
                {/* Render the collection list directly regardless of login state.
                   If logged out, 'isOwner' falls back to false inside the component automatically.
                */}
