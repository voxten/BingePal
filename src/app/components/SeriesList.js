...
import { FiPlus, FiEdit2, FiTrash2, FiFilter, FiX, FiStar, FiSearch } from 'react-icons/fi';
...
const SeriesList = ({ userId }) => {
    ...
    // Helper to determine progress bar and text colors based on percentage
    const getProgressColor = (progress) => {
        ...
        if (progress <= 99) {
            return {
                bar: 'from-violet-500 to-indigo-500', // Your current blue/violet gradient
                text: 'text-indigo-600 dark:text-indigo-400'
            };
        }
        return {
            bar: 'from-emerald-500 to-green-500', // 100% complete
    ...
    if (!user) {
        return (
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">
                    Access Denied
                </h2>
                <p className="text-slate-500 dark:text-slate-400">Please sign in to manage your series collection.</p>
            </div>
        );
    }
    ...
    if (isEmptyCollection) {
        return (
            <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/50 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
                <h3 className="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">Welcome to your tracking space</h3>
                <p className="text-slate-500 dark:text-slate-400 mb-6 max-w-sm mx-auto">Get started by building your ultimate series collection tracker.</p>
                <button
                    onClick={() => setIsAddModalOpen(true)}
                    className="bg-indigo-600 hover:bg-indigo-700 active:scale-98 text-white font-medium px-6 py-3 rounded-xl transition-all shadow-lg shadow-indigo-500/20 text-md"
                >
        ...
        <>
            ...
            {showFilters && (
                <div className="mb-6 p-5 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-md transition-all animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1.5">Min Watched Episodes</label>
                            <input
                                type="number"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-5">
                {filteredSeries?.map((doc) => {
                    ...
                    const progress = Math.min(
                        ...
                        100
                    ) || 0; // Fallback to 0 if totalEpisodes is 0 or undefined

                    // Get the color scheme for the current item's progress
                    const progressColors = getProgressColor(progress);

                    return (
                        <div
                            key={doc.id}
                            className="group flex flex-col bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden hover:shadow-xl hover:border-slate-300/50 dark:hover:border-slate-700/50 transition-all duration-300"
                        >
                            {/* Series Image / Poster */}
                            ...
                            <div className="p-4 flex flex-col flex-grow justify-between bg-white dark:bg-slate-900">
                                    ...
                                    <div className="flex gap-0.5">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button
                                                ...
                                                className="focus:outline-none transition-transform active:scale-130"
                                            >
                                                <FiStar
                                                    className={`w-4 h-4 ${
                                                        star <= (data.rating || 0)
                                ...
                                <div className="mb-4">
                                    <div className="flex justify-between text-xs font-semibold mb-1.5">
                                        ...
                                        <span className={`transition-colors duration-300 ${progressColors.text}`}>
                                            {progress}%
                                        </span>
                                    </div>
                                    <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                                        <div
                                            className={`h-full rounded-full bg-gradient-to-r ${progressColors.bar} transition-all duration-500 ease-out shadow-sm`}
                                ...
                                {/* Divider line before structural actions */}
                                <div className="border-t border-slate-100 dark:border-slate-800 pt-3 flex justify-between items-center">
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400 font-semibold transition-colors text-xs"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-1.5 text-slate-400 hover:text-rose-600 dark:text-slate-500 dark:hover:text-rose-400 font-semibold transition-colors text-xs"
                                    >
            ...
            {filteredSeries?.length === 0 && (
                <div className="text-center py-16 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <button
                        onClick={resetFilters}
                        className="text-xs font-bold uppercase tracking-wider px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-indigo-600 dark:text-indigo-400 shadow-sm"
                    >
